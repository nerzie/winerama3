...
def update_clusters():
    ...
    if num_reviews % update_step == 0: # using some magic numbers here, sorry...
        ...
        all_wine_ids = set([x.wine.id for x in Review.objects.only("wine")])
        ...
        for cluster in list(new_clusters.values()): # clusters need to be saved before referring to users
            cluster.save()
